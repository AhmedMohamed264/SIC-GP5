...
namespace SIC_Backend.Controllers
{
    ...
    public class NotificationsController(ApplicationDbContext dbContext, NotificationService.NotificationService notificationService) : ControllerBase
    {
        [HttpPost]
        [Route("{pin}")]
        public async Task<ActionResult >SendNotificationToDeviceOwner(int pin, string title, string body)
        {
            var user = await dbContext.Users.Where(u => u.Devices.Any(d => d.Pin == pin)).Include(u => u.NotificationTokens).FirstAsync();
